...
#' distance_paths <- distwalk(file, Id_name, timestamp, crs_epsg)
#' head(distance_paths)
...
distwalk <- function(file, Id_name, timestamp, crs_epsg) {
  ...
  formats_to_try <- c(
    "%m/%d/%y %H:%M", "%m/%d/%y %H:%M:%S", "%m/%d/%y", "%d/%m/%y", "%y-%m-%d", "%m-%d-%y", "%d-%m-%y",
    "%y-%m-%d %H:%M:%S", "%m-%d-%y %H:%M:%S", "%d-%m-%y %H:%M:%S",
    "%m/%d/%y %H:%M:%S", "%d/%m/%y %H:%M:%S","%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S",
    "%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%d %H:%M:%OS", "%Y-%m-%dT%H:%M:%OS",
  ...
  Movementpath <- movementsplit[, c("Month", "Year", "Id", "Length_km")]
  head(Movementpath)
