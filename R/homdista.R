homdista <- function(file, formattime, tz, groupid, perc, unarea){
  ...
  data_df <- read.csv(file=file)

  # Change the time format
  data_df$time <- as.POSIXct(data_df$timestamp, format = "formattime", tz="tz", x=x, y=y, crs_epsg)
  ...
  get_vertices <- function(kde) {
    ...
    code_name <- tryCatch({
      getverticeshr(kde, percent = perc, unout = "unarea")
    }, error = function(e) {
  ...
  traveled_distances_df <- do.call(rbind, lapply(names(traveled_distances), function(code) {
    data.frame(Code = code, Distance_km = traveled_distances[[code]], row.names = NULL)
  }))
