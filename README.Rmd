---
title: "Home range utilization and traveled distance per month and year"
output:
  github_document:
    toc: true
  pdf_document: 
...
The homdista package is designed to analyze the movement patterns of objects, like animals, using recorded GPS coordinates. This toolkit offers various functions to compute home range areas and distances traveled over extended periods, months and years. To determine home range, the package employs a kernel density estimator, requiring users to specify the bandwidth themselves. For distance estimation, the package utilizes the st_distance function to link all points in chronological order based on their timestamps.
...
3. Get correlation values and plot the relationship between area and distance values
```
library(homdista)
...
5. Converts a data frame into a move object for more detailed movement analysis
```
library(homdista)
